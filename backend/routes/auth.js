...
authRouter.post("/create", async (req, res, next) => {
  ...
  try {
    ...
    const newAuthInfo = await UserModel.create({
      email,
      hash: newHash,
      salt,
      __link: { collectionName: "userInfo", id: newUser.id },
    });
...
authRouter.post("/login", async (req, res, next) => {
  ...
  try {
    let { hash, salt, ...existential } = await UserModel.findOne({
      email: user.email,
    });
    ...
    if (newHash == hash) {
      if (!donotgenerate)
        token = jwt.sign({ email: user.email, hash: user.hash }, secretkey, {
          expiresIn: "865005s",
        });
      exists.token = token;
